  ...
  },
  {
    ...
    endpoints: [
      ...
      { m: "POST", path: "/plural/signup", auth: "public",
        desc: "Create a non-admin account. Body { username, password (≥10 chars), email, display_name?, turnstile_token }. Email is required and must be unique — it is the only self-service password-recovery route." },
      ...
      { m: "POST", path: "/plural/forgot-password", auth: "public",
        desc: "Request a password-reset link. Body { email, turnstile_token }. Always returns the same message whether or not the address matches an account, so it cannot be used to enumerate users. Rate limited to one request per IP per minute.",
        example: 'POST body: { "email": "you@example.com", "turnstile_token": "…" }  →  { "success": true, "message": "If an account exists for that email address, a reset link is on its way." }' },
